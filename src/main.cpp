...
#if defined(NDEBUG)
# error "litecoindiamond cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd83996e1c5e8de190efe449836c5dd0323383b8b5b7d41f5db394ccdabc0b294");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // litecoindiamond: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "litecoindiamond Signed Message:\n";
...
{
    // litecoindiamond: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // litecoindiamond: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // litecoindiamond: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // litecoindiamond: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // litecoindiamond: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // litecoindiamond: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9b7452b26732f91c1cc3f842d3f277b2b96f6b9d3beb1e8c31a3f9f9bb7ff3cf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "4/12/2022 - NY LT gov brian benjamin resigns after corruption charges";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041228018c6b74c07deaeb323f49e9d3f9de4c35f868fba5006a1afb0964e0b7fdd4e48a5879dea62c12399c455a20dbed335eca746c311c87b4f0de3344aac8d2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1649823473;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084709152;

        if (fTestNet)
        {
            block.nTime    = 1649823433;
            block.nNonce   = 388009336;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x59b633349a68ea5985692316cea9c7386acacd0e99847579b8cdba4b86654757"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xcc, 0xb3, 0xda }; // litecoindiamond: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("litecoindiamondMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("litecoindiamondMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static litecoindiamondMiner(CWallet *pwallet)
{
    printf("litecoindiamondMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litecoindiamond-miner");
    ...
    try { loop {
        ...
        printf("Running litecoindiamondMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("litecoindiamondMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&litecoindiamondMiner, pwallet));
}
